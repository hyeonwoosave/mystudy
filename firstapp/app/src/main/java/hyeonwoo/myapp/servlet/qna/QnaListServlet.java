...
@WebServlet("/qna/list")
public class QnaListServlet extends GenericServlet {
  ...
  public void service(ServletRequest request, ServletResponse response)
    ...
    out.println("<a href='/qna/form.html '>새 리뷰</a>\n");
    ...
    try {
      ...
      List<Qna> list = qnaDao.findAll();

      for (Qna qna : list) {
        out.printf(
            "<tr> <td>%d</td> <td><a href='/qna/view?no=%1$d'>%s</a></td> <td>%s</td> <td>%s</td> <td>%d</td> </tr>\n",
            qna.getNo(),
            qna.getTitle(),
